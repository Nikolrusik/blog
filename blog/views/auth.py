...
def register():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        try:
            ...
            current_app.logger, exception(f"Created user {user}")
            login_user(user)
...
def login_as():
    ...
    if not username:
        return render_template("auth/login.html", errors="username not passed")

    user = User.query.filter_by(username=username).one_or_none()
    if user is None:
        return render_template("auth/login-as.html", error=f"no user {username}")
